...
<h3>每日推荐 <a href="commodity.action?showCommodityRandom4" style="text-align: right;float: right;">换一换</a></h3>
	<div id="clothes">
	<h4>精品服装</h4>
		<c:forEach items="${clothes}" var="clothes">
			<div id="commodity" onclick="chooseCommodity(${clothes.id})">
				<div id="commodity_pic">
					<img src="/file/${clothes.commoditypic}">
				</div>
				<div id="commodity_info">
					<div>${clothes.commodityname}</div>
				</div>
			</div>
		</c:forEach>
	</div>
	...
	<h4>精选点心</h4>
		<c:forEach items="${food}" var="food">
			<div id="commodity" onclick="chooseCommodity(${food.id})">
				<div id="commodity_pic">
					<img src="/file/${food.commoditypic}">
				</div>
				<div id="commodity_info">
					<div>${food.commodityname}</div>
				</div>
			</div>
		</c:forEach>
	</div>
	...
	<h4>热销书籍</h4>
		<c:forEach items="${book}" var="book">
			<div id="commodity" onclick="chooseCommodity(${book.id})">
				<div id="commodity_pic">
					<img src="/file/${book.commoditypic}">
				</div>
				<div id="commodity_info">
					<div>${book.commodityname}</div>
				</div>
			</div>
		</c:forEach>
	</div>
...
<c:if test="${user!=null&&commodityRecommend.size()>0}">
<div id="show_commodity">
<div id="recommend">
	<h3>猜你喜欢 </h3>
	<c:forEach items="${commodityRecommend}" var="commodityRecommend">
			<div id="commodity" onclick="chooseCommodity(${commodityRecommend.id})">
				<div id="commodity_pic">
					<img src="/file/${commodityRecommend.commoditypic}">
				</div>
				<div id="commodity_info">
					<div>${commodityRecommend.commodityname}</div>
				</div>
			</div>
	</c:forEach>
</div>
